...
from src.loom_io.documents import (
    read_docx, read_docx_with_formatting, write_docx,
    read_latex, read_latex_with_structure, write_text_lines,
    read_resume, apply_edits_to_docx, read_text,
    _categorize_edits, _copy_run_formatting, _set_paragraph_text_preserving_format
)
from src.loom_io.generics import write_json_safe, read_json_safe, ensure_parent, exit_with_error
...
class TestDocxRoundTrip:
    ...
    def test_apply_edits_preserves_untouched_formatting(self, simple_docx_path, tmp_path):
        ...
        apply_edits_to_docx(simple_docx_path, new_lines, output_path, preserve_mode="in_place")
        ...
        assert first_run.bold is True or first_run.bold is None, f"Expected bold formatting preserved, got: {first_run.bold}"
...
class TestLatexOperations:
    ...
    def test_read_latex_with_structure_preserves_comments(self, sample_latex_path):
        ...
        # find comment lines
        comment_lines = [text for text in lines.values() if text.strip().startswith('%')]
        assert len(comment_lines) > 0, "Should preserve comment lines"
        
        # verify specific comment is preserved
        assert any("This is a comment that should be preserved" in comment for comment in comment_lines)
    ...
    def test_latex_roundtrip_preserves_structure(self, simple_latex_path, tmp_path):
        ...
        for element in ["\\documentclass", "\\begin{document}", "\\section{", "\\end{document}"]:
...
class TestLineMappingHelpers:
    ...
    def test_categorize_edits_identifies_modifications(self, sample_lines_for_edits):
        ...
        modifications, additions, deletions = _categorize_edits(original_lines, new_lines)
        
        assert 6 in modifications
        assert modifications[6] == "Senior Python developer w/ 7+ years building applications."
...
class TestErrorHandling:
    ...
    def test_apply_edits_to_docx_preserve_modes(self, simple_docx_path, tmp_path):
        ...
        apply_edits_to_docx(simple_docx_path, modified_lines, output_in_place, preserve_mode="in_place")
        
        # test rebuild mode
        output_rebuild = tmp_path / "rebuild.docx"
        apply_edits_to_docx(simple_docx_path, modified_lines, output_rebuild, preserve_mode="rebuild")
...
class TestCrossFormatIntegration:
    ...
    def test_lines_type_consistency_across_formats(self, sample_docx_path, sample_latex_path):
    ...
    def test_read_resume_handles_both_formats_consistently(self, sample_docx_path, sample_latex_path):
