...
class TestProcessModifyOperation:
    ...
    def test_process_modify_operation_multiline(self, replace_range_operation):
        ...
        assert (
            result.content
            == "User-modified multi-line content\nwith specific technical details\nand enhanced formatting"
        )
...
class TestProcessPromptOperationAdvanced:
    ...
    def test_process_prompt_operation_context_building(
        ...
        call_kwargs = (
            mock_build_prompt.call_args[1] if mock_build_prompt.call_args[1] else {}
        )
        call_args = (
            mock_build_prompt.call_args[0] if mock_build_prompt.call_args[0] else []
        )

        # check user instruction preservation
        user_instruction = (
            call_args[0] if call_args else call_kwargs.get("user_instruction")
        )
        assert (
            user_instruction
            == "Make this more technical and focus on machine learning experience"
        )

        # check operation context includes line info
        operation_context = (
            call_args[2] if len(call_args) > 2 else call_kwargs.get("operation_context")
        )
    ...
    def test_process_prompt_operation_missing_instruction(
        self, sample_resume_lines, sample_job_description
    ):
        operation = EditOperation(
            operation="replace_line",
            line_number=5,
        ...
        assert "PROMPT operation requires prompt_instruction to be set" in str(
            exc_info.value
        )
    ...
    def test_process_prompt_operation_with_sections(
        ...
        sections_param = call_kwargs.get("sections_json") or (
            call_args[7] if len(call_args) > 7 else None
        )
...
class TestSpecialOperationsIntegration:
    ...
    def test_error_handling_consistency(self):
        ...
        prompt_op = EditOperation(
            operation="replace_line", line_number=5, prompt_instruction=None  # missing
        )
