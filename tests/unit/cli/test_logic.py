...
class TestArgResolver:
    ...
    def test_resolver_initialization(self):
        settings = LoomSettings(
            data_dir="test_data", output_dir="test_output", model="gpt-4o"
        )
    ...
    def test_resolve_common_uses_defaults(self):
        ...
        assert (
            result["sections_path"] == Path("custom_data") / "sections.json"
        )  # uses custom data_dir
    ...
    def test_resolve_options(self):
        ...
        result = resolver.resolve_options(
            risk=RiskLevel.HIGH, on_error=ValidationPolicy.FAIL_HARD
        )
...
class TestResolveUtility:
    ...
    def test_resolve_returns_provided_when_not_none(self):
        ...
        assert _resolve(Path("provided.txt"), Path("default.txt")) == Path(
            "provided.txt"
        )
...
class TestOrchestrationPipeline:
    ...
    def test_edit_correction_missing_file_error(
        ...
        with (
            patch("src.cli.logic.generate_edits") as mock_generate,
            ...
            def mock_validation_handler(
                settings, validate_fn, policy, edit_fn, reload_fn, ui
            ):
                try:
...
class TestIntegrationScenarios:
    ...
    def test_complete_workflow_input_to_output(
        ...
        resume_lines = {
            1: "John Doe",
            2: "Software Engineer",
            3: "Experience: Python developer",
        }
        job_text = "Looking for Python developer with React skills"
        sections_json = json.dumps(
            {"sections": [{"name": "EXPERIENCE", "start_line": 3, "end_line": 3}]}
        )
        ...
        expected_output = {
            1: "John Doe",
            2: "Software Engineer",
            3: "Experience: Full-stack Python & React developer",
        }
        ...
        assert (
            generated_edits is not None
        )  # ensure generation succeeded before applying
...
class TestCLIHelpers:
    ...
    def test_validate_required_args_missing_values(self):
        with pytest.raises(typer.BadParameter, match="Resume file is required"):
            validate_required_args(
                resume=(None, "Resume file"), job=("job text", "Job description")
            )

        with pytest.raises(typer.BadParameter, match="Job description is required"):
            validate_required_args(
                resume=("resume.docx", "Resume file"), job=(None, "Job description")
            )
    ...
    def test_validate_required_args_empty_strings(self):
        with pytest.raises(typer.BadParameter, match="AI model is required"):
            validate_required_args(model=("", "AI model"))
    ...
    def test_validate_required_args_falsy_values(self):
        ...
        with pytest.raises(typer.BadParameter, match="Number value is required"):
            validate_required_args(number=(0, "Number value"))
    ...
    def test_validate_required_args_error_message_content(self):
        try:
            validate_required_args(custom_arg=(None, "Custom argument description"))
...
class TestOutputDirectoryHandling:
    ...
    def test_path_composition_with_custom_directories(self, tmp_path):
        ...
        assert custom_settings.resume_path == Path(
            tmp_path / "custom_data" / "resume.docx"
        )
        assert custom_settings.sections_path == Path(
            tmp_path / "custom_data" / "sections.json"
        )
        assert custom_settings.edits_path == Path(
            tmp_path / "custom_output" / "edits.json"
        )
        assert custom_settings.warnings_path == Path(
    ...
    def test_output_directory_git_ignore_compliance(self, output_test_setup):
        ...
        loom_files = ["edits.warnings.txt", "diff.patch", "plan.txt"]
...
class TestEditConversion:
    ...
    def test_convert_dict_edits_to_operations_replace_line(
        ...
        operations = convert_dict_edits_to_operations(
            sample_dict_edits, sample_resume_lines
        )
        ...
        assert (
            replace_op.content == "Senior Full Stack Developer at Tech Corp (2020-2023)"
        )
        assert replace_op.reasoning == "Update title to match job requirements"
        assert replace_op.confidence == 0.9
        assert (
            replace_op.original_content == "Senior Developer at Tech Corp (2020-2023)"
        )
    ...
    def test_convert_dict_edits_to_operations_invalid_op_type(
        ...
        operations = convert_dict_edits_to_operations(
            invalid_edits, sample_resume_lines
        )
    ...
    def test_convert_dict_edits_to_operations_missing_lines(self, sample_resume_lines):
        ...
        operations = convert_dict_edits_to_operations(
            edits_with_missing_line, sample_resume_lines
        )
...
class TestSpecialOperations:
    ...
    def sample_sections_json(self):
        return json.dumps(
            {"sections": [{"name": "EXPERIENCE", "start_line": 3, "end_line": 4}]}
        )
    ...
    def test_operations_to_dict_edge_cases(self):
        ...
        operations = [
            ...
            EditOperation(
                ...
                content="test2",
                status=DiffOp.REJECT,
                confidence=0.3,  # rejected operation
            ),
            EditOperation(
                operation="replace_line",
                line_number=3,
                content="test3",
                status=DiffOp.SKIP,
                confidence=0.5,  # skipped operation
            ),
            EditOperation(
                operation="replace_line",
                line_number=4,
                content="test4",
                status=DiffOp.SKIP,
                confidence=0.8,  # no status
            ),
        ]

        original_edits = {"version": 1, "meta": {"model": "gpt-4o"}, "ops": []}
    ...
    def test_special_operations_missing_context(self, mock_process_modify):
        operations = [
            EditOperation(
                ...
                content="MODIFY: test",
                status=DiffOp.MODIFY,
                prompt_instruction="modify instruction",
            )
        ]
