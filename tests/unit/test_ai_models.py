...
from src.ai.models import (
    resolve_model_alias, validate_model, get_model_error_message, ensure_valid_model,
    check_openai_api_key, check_claude_api_key, get_ollama_models, is_ollama_available,
    get_models_by_provider, get_model_provider, get_default_model,
    _is_test_model, _get_test_model_provider,
    OPENAI_MODELS, CLAUDE_MODELS, SUPPORTED_MODELS, MODEL_ALIASES, MODEL_CATEGORIES
)
...
class TestResolveModelAlias:
    ...
    def test_resolve_full_model_name(self):
        ...
        assert resolve_model_alias("claude-opus-4-1-20250805") == "claude-opus-4-1-20250805"
...
class TestGetModelErrorMessage:
    ...
    @patch('src.ai.models.validate_model')
    @patch('src.ai.models.get_ollama_models')
    def test_error_message_ollama_model_missing_with_available(self, mock_get_models, mock_validate):
    ...
    @patch('src.ai.models.validate_model')
    @patch('src.ai.models.get_ollama_models')
    def test_error_message_ollama_model_missing_no_available(self, mock_get_models, mock_validate):
        mock_validate.return_value = (False, "ollama_model_missing")
        mock_get_models.return_value = []
    ...
    @patch('src.ai.models.check_claude_api_key')
    @patch('src.ai.models.check_openai_api_key')
    def test_error_message_comprehensive(self, mock_openai_key, mock_claude_key, 
                                       mock_ollama_avail, mock_providers, mock_validate):
        mock_validate.return_value = (False, "model_not_found")
        mock_providers.return_value = {
            "openai": {"models": ["gpt-5"], "available": True, "requirement": "API key"},
            "claude": {"models": ["claude-opus-4"], "available": False, "requirement": "API key"},
...
class TestEnsureValidModel:
    ...
    @patch('typer.echo')
    def test_ensure_valid_model_invalid(self, mock_echo, mock_error_msg, mock_validate, mock_resolve):
...
class TestGetModelsByProvider:
    ...
    @patch('src.ai.models.get_ollama_models')
    @patch('src.ai.models.is_ollama_available')
    def test_get_models_by_provider_complete(self, mock_ollama_avail, mock_ollama_models, 
                                           mock_claude_key, mock_openai_key):
        mock_openai_key.return_value = True
        mock_claude_key.return_value = False
...
class TestModelConstants:
    ...
    def test_model_aliases_validity(self):
        for alias, target in MODEL_ALIASES.items():
            assert target in SUPPORTED_MODELS, f"Alias '{alias}' points to unsupported model '{target}'"
    
    # * Test category models are valid
    def test_model_categories_validity(self):
        for category, models in MODEL_CATEGORIES.items():
            for model in models:
                assert model in SUPPORTED_MODELS, f"Category '{category}' contains unsupported model '{model}'"