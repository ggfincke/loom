...
def test_config_path_returns_isolated_temp_path(isolate_config):
    ...
    result = runner.invoke(
        app, ["config", "path"], env={"NO_COLOR": "1", "TERM": "dumb"}
    )
...
def test_config_set_get_round_trip(isolate_config):
    ...
    with runner.isolated_filesystem():
        ...
        result = runner.invoke(
            app, ["config", "get", "model"], env={"NO_COLOR": "1", "TERM": "dumb"}
        )
        assert result.exit_code == 0
        ...
        result = runner.invoke(
            app, ["config", "get", "temperature"], env={"NO_COLOR": "1", "TERM": "dumb"}
        )
        assert result.exit_code == 0
...
def test_config_list_shows_newly_set_keys(isolate_config):
    ...
    with runner.isolated_filesystem():
        ...
        runner.invoke(
            app,
            ["config", "set", "data_dir", "/custom/data"],
            env={"NO_COLOR": "1", "TERM": "dumb"},
        )
...
def test_config_reset_restores_defaults(isolate_config):
    ...
    with runner.isolated_filesystem():
        ...
        runner.invoke(
            app,
            ["config", "set", "model", "custom-model"],
            env={"NO_COLOR": "1", "TERM": "dumb"},
        )
        runner.invoke(
            app,
            ["config", "set", "data_dir", "/custom/path"],
            env={"NO_COLOR": "1", "TERM": "dumb"},
        )
        ...
        # Reset to defaults
        result = runner.invoke(
            app, ["config", "reset"], env={"NO_COLOR": "1", "TERM": "dumb"}
        )
        assert result.exit_code == 0
        ...
        # default data_dir
        result = runner.invoke(
            app, ["config", "get", "data_dir"], env={"NO_COLOR": "1", "TERM": "dumb"}
        )
...
def test_config_theme_setting_validation(isolate_config):
    ...
    with runner.isolated_filesystem():
        ...
        result = runner.invoke(
            app, ["config", "get", "theme"], env={"NO_COLOR": "1", "TERM": "dumb"}
        )
        assert result.exit_code == 0
