...
def test_sectionize_success_with_output_json(
...
def test_sectionize_success_with_model_selection(
    isolate_config, sample_files, mock_ai_success
):
    from src.cli.app import app
...
def test_sectionize_latex_uses_handler(isolate_config):
    ...
    with runner.isolated_filesystem():
        ...
        resume_path.write_text(
            sample_latex.read_text(encoding="utf-8"), encoding="utf-8"
        )
...
def test_apply_success_basic(isolate_config, sample_files, mock_ai_success):
    ...
    with runner.isolated_filesystem():
        ...
        edits_file.write_text(
            '{"version": 1, "meta": {"strategy": "rule", "model": "gpt-4o", "created_at": "2024-01-15T10:30:00Z"}, "ops": [{"op": "replace_line", "line": 5, "text": "Updated summary line", "current_snippet": "Original text", "why": "Enhanced for job requirements"}]}'
        )
        ...
        result = runner.invoke(
            ...
            [
                ...
                str(edits_file),
                "--output-resume",
                str(output_dir / "tailored_resume.docx"),
            ],
            env={"NO_COLOR": "1", "TERM": "dumb"},
        )
    ...
    with runner.isolated_filesystem():
        ...
        result = runner.invoke(
            ...
            [
                ...
                str(edits_file),
                "--output-resume",
                str(output_dir / "applied_resume.docx"),
            ],
            env={"NO_COLOR": "1", "TERM": "dumb"},
        )
        ...
        with patch("src.core.pipeline.run_generate", side_effect=ai_mock):
            result = runner.invoke(
                ...
                [
                    ...
                    str(sample_files["resume"]),
                    "--output-resume",
                    str(output_dir / "full_tailored.docx"),
                    "--edits-json",
                    str(output_dir / "full_edits.json"),
                    "--model",
                    "gpt-4o",
                ],
                env={"NO_COLOR": "1", "TERM": "dumb"},
...
def test_apply_with_preserve_formatting_flags(
    isolate_config, sample_files, mock_ai_success
):
    from src.cli.app import app
    ...
    with runner.isolated_filesystem():
        ...
        result = runner.invoke(
            ...
            [
                ...
                str(edits_file),
                "--output-resume",
                str(output_dir / "formatted_resume.docx"),
                "--preserve-formatting",
                "--preserve-mode",
                "strict",
            ],
...
def test_generate_with_sections_and_risk_settings(
    isolate_config, sample_files, mock_ai_success
):
    from src.cli.app import app
    ...
    with runner.isolated_filesystem():
        ...
        sections_file.write_text(
            '{"sections": [{"name": "SUMMARY", "start_line": 5, "end_line": 6}]}'
        )
        ...
        with patch("src.core.pipeline.run_generate", side_effect=ai_mock):
            result = runner.invoke(
                ...
                [
                    ...
                    str(sections_file),
                    "--edits-json",
                    str(output_dir / "risk_edits.json"),
                    "--model",
                    "gpt-4o",
                ],
                env={"NO_COLOR": "1", "TERM": "dumb"},
...
def test_config_list_success(isolate_config):
    ...
    with runner.isolated_filesystem():
        result = runner.invoke(
            app, ["config", "list"], env={"NO_COLOR": "1", "TERM": "dumb"}
        )
