...
def show_quick_usage() -> None:
    ...
    for cmd, desc in commands:
        ...
        if "<" in cmd:
            formatted_arg = cmd.replace("help ", "").replace("<command>", "[dim]<command>[/]")
            console.print(f"  [dim]loom[/] [bold]help[/] {formatted_arg}{' ' * (padding)}[dim]# {desc}[/]")
        else:
            console.print(f"  [dim]loom[/] [bold]{cmd}[/]{' ' * padding}[dim]# {desc}[/]")
    console.print()
