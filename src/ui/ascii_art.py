...
def show_loom_art() -> None:
    ...
    stops = [c for i, c in enumerate(get_active_theme()) if i in (0, 2, 4)]
    if len(stops) < 2:
        # fallback if theme is short
        stops = get_active_theme()[:2]
    ...
    def gradient_line(line: str) -> Text:
        # build multi-stop gradient across line by per-char interpolation
        if not line:
            return Text("")
        n = len(line)
        if n == 1:
