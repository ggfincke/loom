...
def _show_models_list() -> None:
    ...
    for provider_name, info in providers.items():
        ...
        if models:
            for model in models:
                model_text = (
                    f"[loom.accent2]{model}[/]" if available else f"[dim]{model}[/]"
                )
                console.print(f"  {styled_bullet()} {model_text}")
        else:
            if provider_name == "ollama":
                console.print(
                    f"  [dim]Start Ollama server & install models to see available options[/]"
                )
            else:
                console.print(
                    f"  [dim]Set {requirement} to use {provider_display} models[/]"
                )
    ...
    # add usage note
    console.print(
        "[dim]Use any available model with:[/] [loom.accent2]loom tailor --model MODEL_NAME[/]"
    )
    console.print(
        "[dim]Set default model with:[/] [loom.accent2]loom config set model MODEL_NAME[/]"
    )
